...
public class GuiHideArmor extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(btnHelmet = new GuiButton(5, cx - 60, cy - 20, 120, 20, ""));
        this.buttonList.add(btnChest = new GuiButton(4, cx - 60, cy + 5, 120, 20, ""));
        this.buttonList.add(btnLegs = new GuiButton(3, cx - 60, cy + 30, 120, 20, ""));
        this.buttonList.add(btnBoots = new GuiButton(2, cx - 60, cy + 55, 120, 20, ""));
        
        this.buttonList.add(btnArrows = new GuiButton(8, cx - 60, cy + 80, 120, 20, ""));
        this.buttonList.add(btnHideAll = new GuiButton(6, cx - 60, cy + 110, 58, 20, "Hide All"));
        this.buttonList.add(btnShowAll = new GuiButton(7, cx + 2, cy + 110, 58, 20, "Show All"));
        ...
        if (player != null) {
            for (int i = 0; i < 50; i++) {
                ...
                if (isHidden) {
                    ...
                    if (i % 2 == 0) { 
                        mc.theWorld.spawnParticle(EnumParticleTypes.CLOUD, x, y, z, vx * 0.5, vy * 0.5, vz * 0.5);
                    }
                } else {
                    mc.theWorld.spawnParticle(EnumParticleTypes.FIREWORKS_SPARK, x, y, z, vx, vy, vz);
                    if (i % 2 == 0) { 
                        mc.theWorld.spawnParticle(EnumParticleTypes.CRIT_MAGIC, x, y, z, vx * 1.5, vy * 1.5, vz * 1.5);
                    }
                }
            }
            
            if (isHidden) {
                mc.theWorld.playSound(player.posX, player.posY, player.posZ, "random.fizz", 0.5F, 1.5F, false);
            } else {
                mc.theWorld.playSound(player.posX, player.posY, player.posZ, "random.orb", 0.5F, 1.0F, false);
            }
        }
    }
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id == 0) { 
            ...
            updateButtons();
        } else if ((button.id >= 2 && button.id <= 5) || button.id == 8) { 
            int slot = button.id == 8 ? 4 : button.id - 2; 
